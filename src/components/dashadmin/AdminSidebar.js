...
import { BriefcaseBusiness, ChevronRight, FileClock, LayoutDashboard, Menu, Settings, Users, WalletCards, HelpCircle, X } from "lucide-react";
...
export default function AdminSidebar({
    ...
    openPegawaiUser, setOpenPegawaiUser,
    openPerjadinLuar, setOpenPerjadinLuar,
    openBendahara, setOpenBendahara
}) {
        ...
        <>
            ...
            <nav className="mb-4 space-y-1.5 text-sm">
                ...
                <div className="relative">
                    <button
                        type="button"
                        onClick={() => setOpenPegawaiUser(!openPegawaiUser)}
                        aria-expanded={openPegawaiUser}
                        className={`w-full text-left flex items-center justify-between gap-3 rounded-xl px-3 py-3 transition-all duration-300 focus:outline-none focus:ring-0 ${openPegawaiUser ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900'}`}
                    >
                        <span className="flex items-center gap-3"><Users size={17} />Pegawai & Kelola User</span>
                        <motion.span 
                            animate={{ rotate: openPegawaiUser ? 90 : 0 }} 
                            ...
                            >
                                <ul className="mt-1 ml-4 pl-2 border-l-2 border-gray-100 space-y-1 py-1">
                                    <button
                                        ...
                                        className={`w-full text-left flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 ${activeTab === 'pegawai' ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}
                                    >
                                        Pegawai
                                    </button>
                                    ...
                                    <button
                                        ...
                                        className={`w-full text-left flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 ${activeTab === 'manage-users' ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}
                                    >
                                        Kelola User
                                    </button>
                                </ul>
                ...
                <div className="relative">
                    <button
                        type="button"
                        onClick={() => setOpenBendahara(!openBendahara)}
                        aria-expanded={openBendahara}
                        className={`w-full text-left flex items-center justify-between gap-3 rounded-xl px-3 py-3 transition-all duration-300 focus:outline-none focus:ring-0 ${openBendahara ? 'bg-cyan-50 text-cyan-700 font-semibold' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900'}`}
                    >
                        <span className="flex items-center gap-3"><WalletCards size={17} />Bendahara</span>
                        <motion.span 
                            animate={{ rotate: openBendahara ? 90 : 0 }} 
                            transition={{ type: "spring", stiffness: 300, damping: 20 }}
                            className="text-sm"
                        >
                            <ChevronRight size={18} />
                    ...
                    <AnimatePresence>
                        {openBendahara && (
                            ...
                            >
                                <ul className="mt-1 ml-4 pl-2 border-l-2 border-gray-100 space-y-1 py-1">
                                    <li onClick={() => toast.success("Coming Soon!")} className="w-full text-left flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 text-gray-500 hover:bg-gray-50 cursor-pointer focus:outline-none">Lorem Ip</li>
                                    <li onClick={() => toast.success("Coming Soon!")} className="w-full text-left flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 text-gray-500 hover:bg-gray-50 cursor-pointer focus:outline-none">Lorem Ip</li>
                                </ul>
                            </motion.div>
                        )}
            ...
            <div className="space-y-1.5 border-t border-slate-200 pt-3">
                ...
                <button
                    ...
                    className={`w-full text-left flex items-center gap-3 rounded-xl px-3 py-3 transition-all duration-300 text-sm ${activeTab === 'update-log' ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900'}`}
                >
                    <FileClock size={17} /> Update Log
                </button>
